...
- **Start** = the next free gap moving *forward* from now: a gap that already contains
  now starts right there; otherwise the gap whose start is the smallest clockwise
  distance ahead. As gaps get filled, the default advances forward automatically, so
  consecutive adds chain onto the end of the planned day.
...
`nextFreeSlot` returns null and the caller falls back to a 1-hour block at the current
time. This applies to the "+" / "Add time block" entry point; the context-menu
"add here" path (tapping an explicit minute on the canvas) keeps its fixed 1-hour
default since the user has chosen the exact spot.
...
- Gap logic stays centralized in `withUnplannedGaps`; `nextFreeSlot` is a thin,
  well-unit-tested consumer of it (chain-forward, gap clamp, midnight wrap, empty and
  fully-planned days).
